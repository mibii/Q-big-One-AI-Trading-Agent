...
class QBigAgent {
    ...
    private loadState() {
        try {
            if (fs.existsSync(STATE_FILE)) {
                ...
                console.log(`📂 Loaded previous state: Peak Balance = $${this.peakBalance.toFixed(2)}`);
            } else {
    ...
    private saveState(currentBalance: number) {
        if (currentBalance > this.peakBalance) {
            ...
            try {
                fs.writeFileSync(STATE_FILE, JSON.stringify({ peakBalance: this.peakBalance }));
            } catch (e) {}
        }
    }
    ...
    private async runLoop() {
        while (this.running) {
            try {
                ...
                console.log(`\n[${now}] 📊 Eq: $${eq.toFixed(2)} | Peak: $${this.peakBalance.toFixed(2)} | DD: -${drawdown.toFixed(2)}% | Pos: ${activePositions.length}`);
